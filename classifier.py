...
# Run tensorflow session
	...
	with tf.Session() as sess:
		new_saver = tf.train.import_meta_graph('./Data/testsave.meta')
		...
		prediction = sess.run(prediction, feed_dict={inputs: image_input})
                ...
                if (prediction[0][1] > 0.85):
                      print " %(file_name)s ::  Yes - Canola leaf found!" % globals()
                      os.remove(file_name)
                else:
                      print " %(file_name)s ::  Ignored" % globals() 
		#print ('It\'s a weed: {}, It\'s a canola: {}'.format(prediction[0][0], prediction[0][1]))
